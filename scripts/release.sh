...
BUMP_PACKAGE_VERSION="$(node -p "require('./package.json').version")"
BUMP_PLUGIN_VERSION="$(node -e "const fs=require('fs');const data=JSON.parse(fs.readFileSync('openclaw.plugin.json','utf8'));process.stdout.write(data.version)")"
if [[ "$BUMP_PACKAGE_VERSION" != "$VERSION" || "$BUMP_PLUGIN_VERSION" != "$VERSION" ]]; then
  echo "Version bump failed: package.json=${BUMP_PACKAGE_VERSION}, openclaw.plugin.json=${BUMP_PLUGIN_VERSION}" >&2
  exit 1
fi

git add package.json openclaw.plugin.json
git commit -m "Release openclaw-slack-bug-report ${TAG}"
git tag "$TAG"
